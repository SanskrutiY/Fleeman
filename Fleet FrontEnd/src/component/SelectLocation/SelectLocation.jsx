...
import {
  Container, Box, FormControl, FormControlLabel, Radio, RadioGroup,
  Stack, Typography, Card, CardContent, useTheme, Button
} from "@mui/material";
...
const Selectlocation = () => {
  ...
  useEffect(() => {
    ...
    if (storedHubData) {
      const parsed = JSON.parse(storedHubData);
      console.log("📦 Hub data loaded:", parsed);
      setLocations(parsed);
    } else {
      console.warn("⚠️ No hub data in sessionStorage. Redirecting...");
      navigate("/");
    }
  }, []);
  ...
  // ✅ Store selected hubId so it can be used in SelectCarOption
  sessionStorage.setItem("hubId", selectedLocation);
  ...
  return (
    <Container>
          ...
          <RadioGroup value={selectedLocation} onChange={handleChange}>
            <Stack spacing={2} sx={{ mt: 3 }}>
              {locations.map((location) => (
                <Card key={location.hubId} variant="outlined"
                  sx={{
                    p: 3, borderRadius: 3, transition: "0.4s",
                    backgroundColor: selectedLocation === location.hubId ? theme.palette.secondary.light : "#fff",
                    boxShadow: selectedLocation === location.hubId
      ...
      <Box mt={4} display="flex" justifyContent="center" gap={2}>
        <Button variant="contained" onClick={handleNavigate}>Continue Booking</Button>
